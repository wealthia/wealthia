# Wealthia — Telegram Channel Post Templates

Copy and paste into your channel. Replace `[PLACEHOLDERS]`.

---

## Post 1 — Game intro

```
🏙️ WEALTHIA — Build your wealth empire!

💰 Tap to earn coins
🏪 Upgrade Shop for higher tap income
🏦 Bank for offline earnings
🏭 Factory for faster energy recovery
🏆 Compete on the global leaderboard

🎮 Play: https://wealthia.github.io/wealthia/v5.html
🤖 Bot: https://t.me/[BOT_USERNAME]
```

---

## Post 2 — Referral campaign

```
👥 Invite a friend — you get +2,000 Wealth Coins, they get +1,000!

1️⃣ Open the game
2️⃣ Go to Friends → Share invite in Telegram
3️⃣ Friend joins via your link
4️⃣ Automatic reward!

🎮 Start: https://t.me/[BOT_USERNAME]?start=ref_[YOUR_ID]
```

---

## Post 3 — Daily missions

```
📋 Daily Missions refreshed!

New missions every 12 hours:
✅ Tap challenges
✅ City value goals
✅ Building upgrade tasks

🎁 Daily Streak increases your bonus every day!

👉 Play now: https://wealthia.github.io/wealthia/v5.html
```

---

## Post 4 — Leaderboard

```
🏆 TOP EMPIRES THIS WEEK

🥇 [Name 1] — [X] city value
🥈 [Name 2] — [X] city value
🥉 [Name 3] — [X] city value

Where do you rank? Check the Rank tab!

🎮 Play: https://wealthia.github.io/wealthia/v5.html
```

---

## Post 5 — Boost weekend

```
⚡ BOOST WEEKEND!

This weekend only:
💪 2x Tap boost — 150 coins
📈 2x Income boost — 200 coins
⚡ Full Energy — 100 coins

Activate from the Earn tab!

🎮 https://wealthia.github.io/wealthia/v5.html
```

---

## Post 6 — New player guide

```
🆕 Welcome to Wealthia!

Your first 60 seconds:
1. TAP the golden button to earn coins
2. City tab → Upgrade your Shop
3. Tasks tab → Claim your daily reward

Questions? Bot: https://t.me/[BOT_USERNAME]
```

---

## Post 7 — Tournament teaser

```
🏆 FIRST TOURNAMENT COMING SOON!

Tap races with coin prizes:
🥇 Big rewards for top tappers
⏱️ Limited-time events

Prepare now:
📺 Watch ads in Earn tab
👥 Invite friends → +2,000 coins

🎮 Play: https://t.me/[BOT_USERNAME]
📢 Channel: https://t.me/[CHANNEL_USERNAME]
```

---

## Post 8 — Earn / ads tip

```
💸 FREE COINS — Earn tab

📺 Rewarded Ad → +300 coins
📢 Join channel → +500 coins
🤝 Partner tasks → +750 coins

🎮 https://wealthia.github.io/wealthia/v5.html
```

---

## Posting tips

- Post once per day at the same time
- Always include: `https://t.me/[BOT_USERNAME]`
- Pin Post 1 (intro)
- Replace `[BOT_USERNAME]` and `[CHANNEL_USERNAME]` before posting
