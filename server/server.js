...
const { handleBotMessage, handleBotCallbackQuery, setupBotProfile } = require("./bot-commands");
...
app.post("/api/telegram/webhook/:secret", paymentSecurity.createTelegramWebhookGuard({
  ...
  try {
    ...
    if (update.callback_query) {
      await handleBotCallbackQuery(update.callback_query, { telegramApiSafe });
      res.json({ ok: true });
      return;
    }
  } catch (error) {
...
async function sendPushMessage(telegramId, text) {
  ...
  try {
    const webAppUrl = process.env.WEBAPP_URL || "https://wealthia.github.io/wealthia/merge-arena/app/?v=38";
    await telegramApi("sendMessage", {
      chat_id: telegramId,
      text,
      reply_markup: {
